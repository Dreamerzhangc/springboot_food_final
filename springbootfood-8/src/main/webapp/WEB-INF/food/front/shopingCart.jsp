...
<body>
	<div class="container-fluid">
		...
		<div class="row mx-auto" style="width: 90%">
			...
			<div class="row mx-auto text-center"
				...
				<span style="color: white; padding-left: 150px; padding-top: 45px">吃不完的美食</span>
                    <img   src="resources/image/front/logo.jpg" style="height: 70px;width: 150px;margin-left: 120px;margin-top: 8px"/>
				...
				<div class="col-md-12 mx-auto" style="padding-left: 43px">
					...
					<div class="row mx-auto text-center"
										...
										<c:otherwise>
											<c:forEach items="${productInfos}" var="productInfo">
												...
												<tr class="row">
													...
													<input type="hidden" name="<%=s%>+${productInfo.productId}"
														value="${productInfo.productId}" id="<%=s %>" />
													...
													<td class="col-md-1 col-1 text-center"><span
														id="productprice<%=s%>">${productInfo.productPrice }</span>

													</td>

													<td class="col-md-2 col-1 text-center">
														<!-- 点击+1  -1 --> <input type="button" class="sub"
														onclick="sub(<%=s%>)" value="-" style="max-width: 21px;">
														<!-- 判断session商品数量 --> <c:forEach
															items="${sessionScope.carts}" var="cart">
															<c:if test="${cart.productId==productInfo.productId }">
																<!-- 	 显示商品数量 -->
																<input id="productnum<%=s %>" readonly="readonly"
																	value="${cart.productQuantity}"
																	style="max-width: 21px;">
															</c:if>
														</c:forEach> <input
														onclick="add(<%=s%>,'${productInfo.productStock}')"
														type="button" value="+" style="max-width: 21px;">
													</td>
													<td class="col-md-2 col-1 text-center">
														<!-- 总价 --> ¥ <input id="onesubtotal<%=s %>"
														onload="onesubtotal(<%=s %>)" readonly="readonly"
														style="max-width: 80%; border: none;"
														value="${productInfo.productPrice }">
													</td>
													<td class="col-md-1 col-1 text-center">
														<button type="button" class="btn btn-outline-danger"
															onclick="delCart(<%=s%>)">删除</button>
													</td>
						...
						<div style="border: px solid white;" class="float-right col-md-5">
							...
							<button type="button" class="btn btn-outline-danger"
								onclick="clearShopping(event)">清空购物车</button>
							<button type="button" class="btn btn-outline-danger"
								onclick="toshopinglist(event)">继续购物</button>
							<button type="button" class="btn btn-outline-danger"
								onclick="toorderpay(event)">堂食结算</button>
							<button type="button" class="btn btn-outline-danger"
								onclick="toooutrderpay(event)">外卖结算</button>

						</div>
						<div style="border: px solid white; padding-left: 100px"
							class="float-right col-md-4"></div>
						...
						<div
							...
							<ul
								...
								<li><label style="color: red">
										&yen;&nbsp;小计&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label><input
									id="smallTotal" class="subtotal" value="0" readonly="readonly"
									style="width: 60px; border: none; background-color: #CCCCCC"></input></li>
								<li><label style="color: red">
										&yen;&nbsp;配送费&nbsp;&nbsp;</label><input id="roadmoney<%=s%>"
									value="10" readonly="readonly"
									style="width: 60px; border: none; background-color: #CCCCCC"></input></li>
								<li id="sumLi"><label style="color: red">&yen;&nbsp;总价&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
									<input id="totalSum" value="10" readonly="readonly"
									style="width: 60px; border: none; background-color: #CCCCCC;"></input>
								</li>
							</ul>
						</div>
	...
	<script type="text/javascript">
		...
		function toooutrderpay(event) {
			...
			var url = "front/outorderpay?totalprice="+totalprice;
